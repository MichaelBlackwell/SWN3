...
function generateConstrainedStarCoordinates(targetCount: number): Coordinates[] {
  ...
  // Verify neighbor constraint
  for (const coord of coordinates) {
    const neighborCount = countSystemNeighbors(coord, usedCoords);
    if (neighborCount > MAX_SYSTEM_NEIGHBORS) {
      console.warn(`System at (${coord.x}, ${coord.y}) has ${neighborCount} system neighbors (max: ${MAX_SYSTEM_NEIGHBORS})`);
    }
  }
...
function generateSpikeRoutes(systems: StarSystem[]): void {
  ...
  // Phase 1: Build minimum spanning tree
  ...
  // Phase 2: Add some additional routes for redundancy
 ...
 * 1. Every hex on the map is within 3 hexes of a system
 * 2. No system has more than 3 neighboring hexes containing systems
 * 3. Scenario rules (system count, faction count, etc.) are enforced
 */
export function generateSectorWithConfig(config: ScenarioConfig): { sector: Sector; factions: Faction[] } {
  ...
  // Use the new constrained coordinate generation
...
export { hexDistance, getNeighbors, isValidPlacement, countSystemNeighbors };
