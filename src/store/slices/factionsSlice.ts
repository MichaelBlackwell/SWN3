...
import { calculateTurnIncome, calculateUpgradeCost } from '../../utils/factionCalculations';
import {
...
const factionsSlice = createSlice({
  ...
  reducers: {
    ...
    // Add XP to a faction
    addXP: (state, action: PayloadAction<{ factionId: string; amount: number }>) => {
      const { factionId, amount } = action.payload;
      const faction = state.factions.find((f) => f.id === factionId);
      if (!faction) return;

      // Add XP (ensure it doesn't go negative)
      faction.xp = Math.max(0, faction.xp + amount);
    },
    // Set or update a faction's goal
    setGoal: (
      state,
      action: PayloadAction<{
        factionId: string;
        goal: import('../../types/faction').FactionGoal | null;
      }>
    ) => {
      const { factionId, goal } = action.payload;
      const faction = state.factions.find((f) => f.id === factionId);
      if (!faction) return;
    ...
    completeGoal: (state, action: PayloadAction<{ factionId: string }>) => {
      const { factionId } = action.payload;
      const faction = state.factions.find((f) => f.id === factionId);
      if (!faction || !faction.goal) return;
    ...
    updateGoalProgress: (
      ...
      const { factionId, current, metadata } = action.payload;
      const faction = state.factions.find((f) => f.id === factionId);
      if (!faction || !faction.goal) return;

      faction.goal.progress.current = current;
      if (metadata) {
        faction.goal.progress.metadata = {
          ...faction.goal.progress.metadata,
          ...metadata,
        };
      }
    ...
    upgradeAttribute: (
      ...
      action: PayloadAction<{
        factionId: string;
        attributeName: 'force' | 'cunning' | 'wealth';
      }>
    ) => {
      const { factionId, attributeName } = action.payload;
      const faction = state.factions.find((f) => f.id === factionId);
      if (!faction) return;

      const currentRating = faction.attributes[attributeName];
      const upgradeCost = calculateUpgradeCost(currentRating);

      // Validate upgrade is possible
      if (upgradeCost === 0) {
        // Already at max rating (8) or invalid rating
        return;
      }

      // Validate faction has enough XP
      if (faction.xp < upgradeCost) {
        return;
      }

      // Deduct XP
      faction.xp -= upgradeCost;

      // Increment attribute
      faction.attributes[attributeName] = currentRating + 1;

      // Recalculate maxHp when attributes change
      ...
      const newMaxHp =
        4 +
        xpCosts[faction.attributes.force] +
        xpCosts[faction.attributes.cunning] +
        xpCosts[faction.attributes.wealth];

      // Update maxHp and current HP (current HP increases by the difference)
      const hpIncrease = newMaxHp - faction.attributes.maxHp;
