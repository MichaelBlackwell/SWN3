...
export function useTurnPhase() {
  ...
  const handleMarkActionUsed = useCallback(
    (actionType: string) => {
      dispatch(markActionUsed(actionType));
    },
    [dispatch]
  );
  ...
  const handleCommitAction = useCallback(() => {
    ...
    // Commit the action (marks action type as used, allowing more of same type)
  ...
  return {
    ...
    markActionUsed: handleMarkActionUsed,
