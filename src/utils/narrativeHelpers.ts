...
import { getSystemDisplayName } from './systemDisplay';
...
export function createNarrativeContextFromFaction(
  ...
  const rawSystemName = faction.homeworld ? getSystemName(faction.homeworld) : undefined;
  const formattedSystemName = rawSystemName ? getSystemDisplayName(rawSystemName) : undefined;
...
export function createNarrativeContextFromSystem(
  ...
  return {
    systemName: getSystemDisplayName(system.name),
