...
export default function CombatAnimation({
  ...
  // Play combat attack sound on mount
  useEffect(() => {
    playSound('combat_attack');
  }, [playSound]);
  ...
  // Progress through animation phases with sound effects
  useEffect(() => {
    if (phase === 'showing-rolls') {
      const timer = setTimeout(() => setPhase('showing-result'), 1000);
      return () => clearTimeout(timer);
    } else if (phase === 'showing-result') {
      // Play hit/miss/critical sound based on result
      const { attackerWins, bothSucceed } = combatResult;
      
      if (bothSucceed) {
        playSound('combat_critical'); // Tie = both hit
      } else if (attackerWins) {
        // Check if it's a kill (asset destroyed) - use the prop passed in
        if (defenderHpAfter <= 0) {
          playSound('combat_destroyed');
        } else {
  ...
  }, [phase, onComplete, combatResult, playSound, defenderHpAfter]);
