...
export default function MainMenu() {
  ...
  const handleStartTutorial = () => {
    // Initialize a basic sector for the tutorial
    const newSector = generateSector();
    dispatch(setSector(newSector));
    
    // Clear existing factions and turn state
    dispatch(clearAllFactions());
    dispatch(resetTurnState());

    // Create a player faction for the tutorial
    if (newSector.systems.length > 0) {
      const firstSystem = newSector.systems[0];
      const playerFaction = generateRandomFactionForSystem(firstSystem);
      playerFaction.name = 'Player Faction';
      playerFaction.attributes = {
        force: 4,
        cunning: 4,
        wealth: 4,
        hp: 20,
      ...
      dispatch(addFaction(playerFaction));
    }

    // Start tutorial and enter editor mode
      ...
      <div className="main-menu__container">
        ...
        <div className="main-menu__buttons">
          ...
          >
            ...
            <div className="main-menu__button-content">
              <span className="main-menu__button-title">Tutorial</span>
              <span className="main-menu__button-description">
                Learn the basics of faction management
              </span>
            </div>
          </button>
