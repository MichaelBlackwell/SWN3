...
export default function ExpandInfluenceModal({
  ...
  const validTargets = getValidExpandTargets<StarSystem>(faction, systems);
  ...
  */
  ...
  const handleConfirm = () => {
    ...
    dispatch(tutorialEventOccurred({ eventId: 'influenceTutorial.expandConfirmed' }));
  };
    ...
    <div className="expand-influence-modal-overlay" onClick={onClose}>
      <div className="expand-influence-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {validTargets.length === 0 ? (
              ...
              <div className="expand-influence-targets">
                {validTargets.map((system: StarSystem) => {
                  ...
                  const displayName = getSystemDisplayName(system.name);

                  return (
                    <div
                      key={system.id}
                      className={`expand-influence-target target-world-card ${isSelected ? 'selected' : ''}`}
                      onClick={() => {
                        setSelectedSystemId(system.id);
                        dispatch(tutorialEventOccurred({ eventId: 'influenceTutorial.targetSelected' }));
                      }}
                    >
        ...
        <div className="expand-influence-modal-footer">
          ...
          <button
            className="expand-influence-confirm-btn confirm-btn"
