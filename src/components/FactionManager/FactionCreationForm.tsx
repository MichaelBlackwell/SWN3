...
export default function FactionCreationForm() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const selectedSystem = systems.find((system: StarSystem) => system.id === formData.homeworld);
    if (!selectedSystem) {
    ...
    if (formData.goal) {
      newFaction.goal = createFactionGoal(formData.goal as FactionGoalType, newFaction);
    }
            ...
            {systems.map((system: StarSystem) => {
              ...
              return (
                <option key={system.id} value={system.id}>
                  {displayName} ({system.coordinates.x}, {system.coordinates.y})
                </option>
              );
            })}
          </select>
